<!doctype html>
<html lang="ru">
<head>
	<meta charset="UTF-8">
	<title>Верстка</title>
	<meta name="viewport" content="width=device-width, shrink-to-fit=no" />	
	<link rel="stylesheet" href="css/grid.css" />	
	<link rel="stylesheet" href="css/fonts.css" />	
	<link rel="stylesheet" href="css/font-awesome.css" />
	<link rel="stylesheet" href="libs/ion-range-slider/ion.rangeSlider.css" />
	<link rel="stylesheet" href="css/style.css" />
</head>
<body>
	<header class="header">		
		<div class="container">
			<div class="row align-items-md-center">
				<div class="col-md-3 order-md-0 col-xl-3">
					<a href="/" class="logo header__logo">
						<span class="logo__main">Pyroboom</span>
						<span class="logo__caption">Интернет-магазин пиротехники</span>
					</a>
				</div>				
				<div class="col-md-4 order-md-2 col-xl-3 header__line">
					<span class="menu-btn header__menu-btn">Меню</span>
					<div class="phone-select header__phone">
						<div class="phone-select__head">(066) 205-07-51</div>
						<ul class="phone-select__list">
							<li><a href="tel:0662050751">(066) 205-07-51</a></li>
							<li><a href="tel:0662050751">(066) 205-07-51</a></li>
						</ul>
					</div>
				</div>
				<div class="col-md-9 col-xl-6 order-md-1">
					<ul class="header__top-menu">
						<li><a href="#">Доставка и оплата</a></li>
						<li><a href="#">Сертификаты качества</a></li>
						<li><a href="#">Контакты</a></li>
						<li><a href="#">Партнёры</a></li>
					</ul>
				</div>
				<div class="col-xl-4 col-md-5 order-md-3 order-xl-5  header__tools">			
					<a href="#" class="cart-btn header__cart">Корзина <span>2</span></a>
					<form action="" class="search-form header__search">
						<input type="search" class="search-form__input">
						<button class="search-form__btn icon-magnify"></button>
					</form>					
				</div>
				<div class="col-xl-12 col-md-12 order-md-4 order-xl-3 header__auth">
					<a href="#">Регистрация</a>
					<a href="#">Вход</a>
				</div>
				<div class="col-xl-8 col-md-12 order-md-5 order-xl-4">
					<ul class="main-menu header__bottom-menu">
						<li class="has-children">
							<a href="#"><i class="icon-rocket"></i>Салютные установки</a>
							<ul>
								<li><a href="#">Салюты маленькие</a></li>
								<li><a href="#">Салюты средние</a></li>
								<li><a href="#">Салюты большие VIP</a></li>
								<li><a href="#">Фейерверки разнокалиберные</a></li>
								<li><a href="#">Миномёты</a></li>
							</ul>
						</li>
						<li class="has-children">
							<a href="#"><i class="icon-dynamite"></i>Мелкая пиротехника</a>
							<ul>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
							</ul>
						</li>
						<li class="has-children">
							<a href="#"><i class="icon-snowflake"></i>Праздничная продукция</a>
							<ul>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
								<li><a href="#">lorem ispum</a></li>
							</ul>
						</li>
						<li><a href="#"><i class="icon-magic-wand"></i>Шоу «под&nbsp;ключ»</a></li>
					</ul>
				</div>				
			</div>
		</div>
	</header>
